<template>
	...
	>
		...
		<v-card-title>
			<v-row>
				<v-col class="text-left" cols="4">{{product.long_name}} {{baseProduct.long_name}}</v-col>
				...
				<v-tabs-items v-model="tabsProduct">
					<v-tab-item value="general">
						<v-row>
							...
							<v-col cols="4">
								<v-img :src="product.images[0] == undefined ? '' : product.images[0].url" ></v-img>
							</v-col>
						</v-row>
					</v-tab-item>
						...
						<v-row>
							...
							<v-col cols="6">
								...
								<v-form>
									<v-row>
										<v-col cols="3">
											<v-text-field :value="product.applied_attributes === undefined ? '' : product.applied_attributes[0].value " :disabled="isUpdating" outline label="Talla"></v-text-field>
										</v-col>
										<v-col cols="3">
											<v-text-field :value="product.applied_attributes === undefined ? '' : product.applied_attributes[1].value" :disabled="isUpdating" outline label="Color"></v-text-field>
										</v-col>
									</v-row>
								</v-form>
							</v-col>
